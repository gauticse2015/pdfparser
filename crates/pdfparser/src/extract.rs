...
fn try_full_page_render(
    ...
    match renderer.render_gray(page_index as u32, 150, &safety) {
        Ok(rp) => Some(rp),
        Err(_) => None, // fail-soft: continue with vector + embedded
    }
}
...
";
    ...
    fn form_grid_lattice_detects() {
        ...
        let (tabs, diag) = detect_tables_page_with_diagnostics(
            0,
            &pc.runs,
            &pc.rules,
            &opts,
