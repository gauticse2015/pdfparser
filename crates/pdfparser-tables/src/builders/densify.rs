...
fn multi_col_band_centers(
    ...
    text_band_centers(runs, frame_x0, frame_x1, y_ttb, min_cell, /* multi_only */ true)
}
...
fn all_text_band_centers(
    ...
    text_band_centers(runs, frame_x0, frame_x1, y_ttb, min_cell, /* multi_only */ false)
}
...
pub(crate) fn expand_xs_exterior_text_cols(
    ...
    let mut left_ext: Vec<f32> = exterior
        .iter()
        .copied()
        .filter(|&c| c < x0)
        .collect();
    ...
    let mut right_ext: Vec<f32> = exterior
        .iter()
        .copied()
        .filter(|&c| c > x1)
        .collect();
    ...
    if !right_ext.is_empty() {
        ...
        if join > x1 + min_cell * 0.25
            && !out.iter().any(|&x| (x - join).abs() < min_cell * 0.5)
        {
            out.push(join);
        }
...
pub(crate) fn densify_y_from_text_bands(
    ...
    let band_centers = if all_gate
        || (!all_centers.is_empty() && multi_centers.len() * 2 >= all_centers.len())
    {
        all_centers
    } else {
        multi_centers
