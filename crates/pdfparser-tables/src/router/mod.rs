...
use crate::policy::{
    is_nested_table_pair, overlaps_owned, pad_rect, rect_iou, ProposalPolicy,
};
...
fn merge_pair(a: &RegionProposal, b: &RegionProposal) -> RegionProposal {
    ...
    let origin = if a.origin == b.origin {
        ...
        if matches!(a.origin, ProposalOrigin::Detector) || matches!(b.origin, ProposalOrigin::Detector)
        {
...
mod tests {
    ...
    fn partition_rejects_tiny_corner_under_outer() {
        ...
        assert_eq!(out.len(), 1, "tiny corner blocked by outer ownership: {out:?}");
